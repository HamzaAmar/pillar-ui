...
```ts
import useTimeout from '@pillar-ui/hooks/useTimeout'
```
...
```tsx
mport useTimeout from '@pillar-ui/hooks/useTimeout'

function MyComponent() { const handleTimeout = () => { console.log('Timeout executed') }
