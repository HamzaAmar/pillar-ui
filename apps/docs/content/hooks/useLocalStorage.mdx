...
```ts
import useLocalStorage from '@pillar-ui/hooks/useLocalStorage'
```
...
function MyComponent() {
  ...
  return (
    <div>
      {' '}
      <p>Stored value: {value}</p> <button onClick={handleClick}>Set Value</button> <button onClick={handleRemove}>
        Remove Value
      </button>{' '}
    </div>
  )
