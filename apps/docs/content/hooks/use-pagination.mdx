...
directory: usePagination/index.ts
...
- [Why usePagination](#why-usepagination)
...
- **initial** (optional): the current page number (default: 1).
- **max**: the total number of pages.
- **perView** (optional): the maximum number of page links to show at once (default: 5).
...
You can install @pillar-ui/hooks using either `npm` or `yarn`.
...
function Pagination() {
  const { range, step, hasNext, hasPrevious, next, previous, goTo } = usePagination({
    initial: 1,
    max: 10,
    perView: 5,
  })
...
If you are experiencing issues with usePagination, ensure that you are passing in the correct values for the max and perView parameters. Also, make sure that you are using the range, step, and navigation functions correctly in your component.
