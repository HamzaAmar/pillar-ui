...
async function addUseClient(filePath, parent) {
  try {
    if (parent && parent[0] === '@') {
...
export async function injectUseClient(dir = DIST_PATH, parent = null) {
  try {
    ...
    for (const item of items) {
      ...
      if (isDirectory) {
        await injectUseClient(itemPath, item)
      } else if (isFile && (item === 'index.js' || item === 'index.mjs')) {
        await addUseClient(itemPath, parent)
      }
    }
